...
public class TimeServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        ByteBuf respByteBuf = Unpooled.copiedBuffer(respMsg, CharsetUtil.UTF_8);
        FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, HttpResponseStatus.OK,respByteBuf);
        response.headers().set(HttpHeaderNames.CONTENT_TYPE,"text/plain");
        response.headers().set(HttpHeaderNames.CONTENT_LENGTH,respByteBuf.readableBytes());

        ctx.write(respByteBuf);//write之后只是会写到缓冲区，调用writeAndFlush可以直接写入SocketChannel
